...
import { cancelFriendRequest } from '../../redux/api/auth'
...
const SentRequests = (props) => {
  ...
  return(
    <Container style={styles.mainContainer}>
    	<Header backEnd blackBackBtn nextScreen="Footer" heading="Sent Friends Requests" navigation={props.navigation} centerHeading/>
      <Content>
        <View style={styles.body}>
          {(!props.auth.requestData.outgoing_requests || !props.auth.requestData.outgoing_requests.length) && 
            
            <View style={styles.noDataCon}>
              <NoData title={convertText("profile.no_outgoing_req", lang)} />
              {/*<Text style={{fontSize: 18}}><FormatText variable='profile.no_outgoing_req'/></Text>*/}
            </View>
          }
          {props.auth.requestData.outgoing_requests && props.auth.requestData.outgoing_requests.map((item, index) => {
